...
module.exports = async (options) => {
  const def_path =
    options.data_model_definitions.length === 0
      ...
      : options.data_model_definitions.map((folder) =>
          path.normalize(process.cwd() + "/" + folder)
        );
  ...
  const gqs_code_gen_path = path.normalize(
    __dirname.slice(0, -4) +
  ...
  for (let p of def_path) {
    if (p.slice(-5).toLowerCase() === ".json") {
      ...
      await fs.copy(p, tmp_dir + path.sep + arr[arr.length - 1]);
    } else {
      let args = [gqs_code_gen_path, "-f", p, "-o", output_dir];
