...
module.exports = async (name, options) => {
  ...
  await fs.copy(
    path.normalize(process.cwd() + `/${name}/2021-12-08T17_37_17.804Z#keycloak.js`),
    path.normalize(`${name}/graphql-server/migrations/2021-12-08T17_37_17.804Z#keycloak.js`)
  );
  await fs.remove(path.normalize(process.cwd() + `/${name}/data_model_definitions/.gitkeep`));
  await fs.remove(path.normalize(process.cwd() + `/${name}/setup.sh`));

  await fs.createFile(path.normalize(`${name}/graphql-server/data.db`));

  await fs.copy(path.normalize(process.cwd() + `/${name}/.env.example`), path.normalize(process.cwd() + `/${name}/.env`));

  const env_path = path.normalize(__dirname.slice(0, -4) + "/env");
  await fs.copy(path.normalize(`${env_path}/.env.graphql_server`), path.normalize(process.cwd() + `/${name}/graphql-server/.env`));
  await fs.copy(path.normalize(`${env_path}/.env.development.spa`), path.normalize(process.cwd() + `/${name}/single-page-app/.env.development`));
  await fs.copy(path.normalize(`${env_path}/.env.production.spa`), path.normalize(process.cwd() + `/${name}/single-page-app/.env.production`));
  await fs.copy(path.normalize(`${env_path}/.env.development.graphiql`), path.normalize(process.cwd() + `/${name}/graphiql-auth/.env.development`));
  await fs.copy(path.normalize(`${env_path}/.env.production.graphiql`), path.normalize(process.cwd() + `/${name}/graphiql-auth/.env.production`));

  log("install graphql-server packages \n");
  let log_gqs = fs.openSync(path.normalize(process.cwd() + `/${name}/logs/graphql-server.log`), "w");
  ...
  let log_spa = fs.openSync(path.normalize(process.cwd() + `/${name}/logs/single-page-app.log`), "w");
  ...
  let log_giql = fs.openSync(path.normalize(process.cwd() + `/${name}/logs/graphiql.log`), "w");
  await spawn_log(false, "yarn", ["install"], {
    detached: true,
