...
module.exports = async (name, options) => {
  ...
  await spawn_console(
    "cp",
    ["./2021-12-08T17_37_17.804Z#keycloak.js", "./graphql-server/migrations"],
    {
      cwd: process.cwd() + `/${name}`,
    }
  );
  ...
  const env_path = __dirname.slice(0, -4) + "/env";
  
  await spawn_console("cp", [".env.example", ".env"], {
    cwd: process.cwd() + `/${name}`,
  });
  ...
  await spawn_console(
    "cp",
    [`${env_path}/.env.development.graphiql`, ".env.development"],
    {
  ...
  await spawn_console(
    "cp",
    [`${env_path}/.env.production.graphiql`, ".env.production"],
    {
      cwd: process.cwd() + `/${name}/graphiql-auth`,
    }
  );
