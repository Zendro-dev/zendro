...
// The sub-projects' `start`/`dev` scripts are cross-platform (the SPA reads
// its port from $PORT via scripts/next-port.js, graphiql-auth runs plain
// node/nodemon), so there is no longer a platform-specific "start:win"/"dev:win"
// split to select between.
const [ startScript, devScript ] = [ "start", "dev" ];
