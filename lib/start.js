...
module.exports = async (service, options) => {
  ...
  for (let serv of service) {
    if (serv === "gqs") {
      ...
      if (prod) {
        spawn_log(true, "bash", [path.normalize("./migrateDbAndStartServer.sh")], {
          detached: true,
          stdio: ["ignore", log_gqs, log_gqs],
          cwd: path.normalize(process.cwd() + "/graphql-server"),
        });
      } else {
        spawn_log(true, "bash", [path.normalize("./migrateDbAndStartServer.sh"), "dev"], {
          detached: true,
          stdio: ["ignore", log_gqs, log_gqs],
          cwd: path.normalize(process.cwd() + "/graphql-server"),
        });
      ...
      let log_giql = fs.openSync(path.normalize(process.cwd() + "/logs/graphiql.log", "a"));
      log("start graphiQL... \n");
      if (prod) {
