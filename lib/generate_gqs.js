...
module.exports = async (options) => {
    ...
    const gqs_code_gen_path = __dirname.slice(0, -4)+'/node_modules/graphql-server-model-codegen/index.js'
    ...
    for (let path of def_path) {
        if (path.slice(-5).toLowerCase()==='.json'){
            ...
            let args = [gqs_code_gen_path, '-f', path, '-o', output_dir].concat(options.args)
    ...
    if (files.length){
        let args = [gqs_code_gen_path, '-f', tmp_dir, '-o', output_dir].concat(options.args)
        if (migrations){
            args.push('-m')
        }
        await(spawn_console('node', args))
