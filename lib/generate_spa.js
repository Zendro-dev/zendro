...
module.exports = async (options) => {
    ...
    // const spa_code_gen_path = process.cwd()+'/single-page-app-codegen/index.js'
    const spa_code_gen_path = __dirname.slice(0, -4)+'/node_modules/single-page-app-codegen/index.js'
    ...
    for (let path of def_path) {
        if (path.slice(-5).toLowerCase()==='.json'){
            ...
            let args = [spa_code_gen_path, '-f', path, '-o', output_dir].concat(options.args)
    ...
    if (files.length){
        let args = [spa_code_gen_path, '-f', tmp_dir, '-o', output_dir].concat(options.args)
        if (create_dir){
            args.push('-D')
        }
