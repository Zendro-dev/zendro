...
module.exports = async (service, options) => {
  ...
  const pidDir = path.normalize(projectDir + "/" + PID_DIR);

  // A dockerized project has no local server pids - its services run in
  // containers started by `zendro dockerize -u`. Detect that so we point the
  // user at the right teardown command instead of printing per-service
  // manual-kill hints for ports that docker (not a local process) owns.
  const dockerized = ["docker-compose-dev.yml", "docker-compose-prod.yml"].some(
    (file) => fs.existsSync(path.normalize(projectDir + "/" + file))
  );
  if (dockerized) {
    log(
      `This looks like a dockerized project (docker-compose-*.yml present).\n` +
        `zendro start/stop only manage non-docker servers; to stop docker ` +
        `services run:\n` +
        `  zendro dockerize -d      (add -p for production, -v to also remove ` +
        `volumes)\n`
    );
  }
  ...
  for (let serv of service) {
    ...
    if (!fs.existsSync(pidFile)) {
      // Suppress the per-service manual hint for docker projects - the docker
      // teardown message above already covers them. Still handle any real pids
      // below in case of a mixed state.
      if (!dockerized) manualStopHint(serv);
      continue;
    }
