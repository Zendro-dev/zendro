...
module.exports = async (name, options) => {
  ...
  if (composeFile) {
    ...
    // Without-docker projects have no containers to tear down, but they DO
    // leave detached node dev servers running (see lib/start.js). The SPA's
    // `next dev` keeps writing into single-page-app/.next/server, which races
    // the deletion below and throws ENOTEMPTY. Stop the servers first, the
    // same way `docker compose down` stops them for docker projects.
    log(`Stop running Zendro servers (gqs, giql, spa)...\n`);
    // rm runs from the project's PARENT directory, so tell stop where the
    // project (and its .zendro pid files) actually is - otherwise it would look
    // in the parent, find no pids, and fall back to unreliable port matching.
    await stop([], { prod: options.prod, projectDir: projectPath });
  }

  log(`Remove project directory: ${projectPath}\n`);
  try {
    // Native fs.rm (re-exported by fs-extra) with retries: even after the
    // servers are stopped a straggler write can briefly repopulate a
    // directory between fs emptying it and rmdir'ing it. fs.remove() can't
    // pass maxRetries/retryDelay, so use fs.rm directly. force:true also
    // swallows ENOENT if a path vanishes underneath us.
